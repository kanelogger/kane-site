...
> **我的评价**：Jev 不适合做复杂任务，但可以做模型路由、工具选择这类判断，放进 dynamic workflow 里。
>
> 它的设计类似「枚举 + switch 函数」，适合的场景有三个共同点：输入可以表示为文本或结构化状态；输出可以提前枚举；调用频繁。
>
> 和现在常用的 LLM（GPT、Claude）相比，训练设计不同。human in the loop 的反馈天然会「讨好人」，TypeSafe 认为 RLHF 是一条弯路：收集人类的主观偏好，再针对偏好使劲优化。所以他们自研了 RLCD。简单说，RLHF 的奖励模型把人类偏好当分数，分数越高越好；RLCD 则把「有把握的程度」当概率来优化，让置信度尽量与真实正确率对齐。
>
> 大厂如果跟进，速度应该会很快，而且大概率会直接做进 Agent，成为 Agent 自带的能力。往后模型路由和 Agent 可能越来越绑定。
