...
category: "AI 编程"
tags: ["Coding Agent", "Token 成本", "上下文压缩", "多 Agent"]
featured: false
order: 4
author: "Kane"
---
...
第一，**会话越长，后续每一轮越贵**。历史不是"存在那里等模型自己想起来"，而是很可能在每轮都要重新带过去。
...
| 推理 Token / thinking budget | 模型内部用于思考和规划的预算 | 简单任务走高推理档位，会明显溢价 |
| 工具往返成本 | 工具描述、调用参数、返回结果进入上下文 | 一次工具调用，常常比原问题还长 |
| 重试成本 | 第一次做错后重新来一轮 | 每修一次，都可能把整包上下文再付一遍 |

这里最容易被低估的，往往是后两项。

**工具往返成本**的问题在于：你以为自己只是"让它查个文件"，但模型经历的其实是：理解工具定义 → 生成参数 → 接收返回值 → 再结合返回值继续思考。对人来说只是一个动作，对系统来说是多段上下文交换。
...
很多人第一次听到缓存，会以为它缓存的是"答案"。其实更接近的理解是：**缓存的是稳定前缀的处理结果**。也就是说，如果两次请求前缀几乎一样，服务端就不必每次都从头处理那一大段相同内容。[1][2]
...
flowchart LR
    subgraph R1["请求 1"]
        P1["系统提示词 / Tool 定义 / 项目背景"] --> A["本轮问题 A"]
    end
    subgraph R2["请求 2"]
        P2["系统提示词 / Tool 定义 / 项目背景"] --> B["本轮问题 B"]
    end
    P1 -. "相同前缀，可复用" .-> P2
```
...
**第一，它省的不是首次成本，而是重复成本。** 第一次发送长前缀时，通常还是要正常付费；价值在第二次、第三次以及后续很多次复用时才会体现出来。

**第二，缓存不是"写短"，而是"写稳"。** 你天天改系统提示词、天天调 Skill Prompt，那缓存理论上存在，实践里也很难命中。

**第三，缓存优化和上下文治理是一回事。** 减少前缀抖动、把稳定内容前置、把变化内容后置，本质都在提升"可复用比例"。

所以 Prompt Cache 本质上不是让模型更聪明，而是让你别为同一段前缀反复买单。OpenAI 与 Anthropic 都提到，长前缀命中缓存后，输入成本和延迟都可能显著下降。[1][2][3]

> **Token 优化重点，不是把提示词写短，而是把前缀写稳。**

（缓存怎么落地，和第四章的 headroom、Session 治理直接相关，后面会展开。）
...
| 层级 | 主题 | 作用 | 特点 | 章节 |
| --- | --- | --- | --- | --- |
| 第一层 | 使用习惯 | 先减少无意义的上下文 | 零成本，立刻可做 | 第二章 |
| 第二层 | 模型路由 | 让不同任务匹配不同模型 | ROI 最高 | 第三章 |
| 第三层 | Context 工程 / 压缩工具 | 稳定前缀、把必进上下文的内容压短 | 装完即用，立即见效 | 第四章 |
| 第四层 | 代码图谱 | 让检索不再盲搜、缩短检索链路 | 系统级优化 | 第五章 |
| 第五层 | Agent 架构 | 把任务分给对的单元，拆开上下文 | 提升长期可维护性 | 第六章 |

它们不是并列的小技巧，而是一条从便宜到贵、从易到难的升级路径：

- **使用习惯**解决的是"无意义的历史和废 Token"
- **模型路由**解决的是"贵模型干便宜活"
- **Context 工程**解决的是"同样前缀/相同内容重复传输"
- **代码图谱**解决的是"每次都从零找代码"
...
没有数据，优化就会变成体感。你觉得自己"省了"，但不知道省在哪，也不知道是不是用质量换的。这条线到第三章 3.6 和第四章的统计命令里，会一再出现。
...
如果你现在就要开始省 Token，最先动的不是架构，不是知识图谱，而是使用习惯。

这一层几乎零工程投入，却经常拿到第一波最大收益。接下来的九个习惯，都是今天就能改的。
...
修 Bug 开一个，做重构开一个，写文章开一个，查线上问题开一个。别混着来，Topic 切分就是最便宜的 Context 管理。
...
Caveman 这类思路受欢迎（见 4.2），不是因为它"简陋"，而是直接打掉高频废 Token。
...
高频、稳定、通用的 Skill 常驻；低频、长说明、低复用的按需触发。和后端服务治理同理：常驻能力要尽量少而精。
...
- [**tapd-ai-cli**](https://github.com/studyzy/tapd-ai-cli)：腾讯 TAPD 的 AI Agent 专用 CLI，支持需求、缺陷、任务的查询和更新。直接用 `tapd story list`、`tapd bug create` 之类的命令操作 TAPD，比拉一套 MCP 轻得多，还能用 `tapd skill init` 一键生成 SKILL.md 配置。
- [**gongfeng-cli**](https://github.com/studyzy/gongfeng-cli)：腾讯工蜂代码托管的 AI Agent 专用 CLI，专门针对 Agent 的认知模式优化了输出格式（Markdown + JSON 混合），支持 PR 创建、代码评审、项目管理等完整工作流，Token 消耗显著低于同等 MCP 方案。

这两个工具的共同思路值得借鉴：**专为 Agent 设计，而不是给人看的 UI 套了个 CLI 壳**。输出格式、信息密度、命令发现机制，都针对 AI 的调用模式做了优化。

MCP 的价值在于：跨系统编排、结构化返回、权限统一收敛、内部业务能力包装。
...
表面上看是"正常沟通"，实际上每一轮来回都在消耗 Token：重新组装上下文、重新带入历史、重新确认状态。四轮对话的成本，往往是一次完整表达的好几倍。
...
斜杠命令（Slash Command）和 Agent 同样支持模型参数绑定，在定义时就固化好模型选择。这样团队里每个人用 `/commit`、`/gen-ut` 这类命令时，都默认走便宜模型，不需要依赖个人记得切换。
...
前面全是"该怎么省"，这一节讲"怎么知道省了多少"。

先建立最简单的三件套：

1. **每个模型有单价基线**：输入/输出的单价 × 实际 Token 量，是一切优化的参照。
2. **重试率是隐藏成本源**：同一任务重跑的次数，直接把单次成本翻倍。
3. **用好工具的统计命令**：第四章的工具几乎都自带统计——`rtk gain` 看命令输出压缩了多少，`headroom_stats` 看压缩率，context-mode 的 `/ctx-stats` 按工具分类看节省。

更进一步，给 Token、成本、重试率、工具调用次数建观测面板，按任务、按模型、按工具分类。省没省、省在哪，数据说话，而不是靠体感。

---

## 第四章 上下文压缩：先把高频上下文压下来

如果说前三章解决的是"少带无关上下文"，这一层解决的就是另一件事：**把那些一定会进上下文的内容，尽量压短。**
...
Agent 执行 `cargo test`，成千上万行测试日志、警告、进度条、ANSI 颜色码全部涌进上下文。
...
> **说明**：这张表混合了 token 和字符两种统计口径：前几行是 token，`vitest run` 这一行是字符长度，用来说明压缩幅度，不能和 token 行直接横向比较。另外，RTK 压的是"命令输出送往上下文"的量，它只是输入 Token 的一部分，且官方按 bytes/4 估算 token（百分比可靠、绝对数字近似）——所以账单的实际降幅会小于压缩率，它的价值是把最大的单点噪声一次性打掉。[8]
...
**注意**：安装后用 `rtk gain` 验证节省面板能打开。如果命令不存在，可能装了同名的 Rust Type Kit，需卸载重装。[8]
...
rtk init -g --agent codebuddy  # CodeBuddy（Fork 版）
```

重启 Agent 后自动生效，所有命令透明经过 RTK 过滤，开发者无感知。
...
**核心数据**（来自 arXiv:2604.00025）：[10][11]
...
Caveman 官方目前还没有 CodeBuddy 支持，使用 [studyzy Fork 版](https://github.com/studyzy/caveman)（已增加 CodeBuddy 支持）：
...
headroom 不只是压缩器，它首先是代理层：Agent 不直接连 LLM，而是先经过本地的 headroom 代理入口，提示词、工具输出、日志、RAG 结果、文件等上下文都先在这里被接住、处理、再转发。代理层内部是一条处理流水线：

```mermaid
flowchart LR
    agent["Agent 所有上下文入口"] --> align["CacheAligner<br>稳定前缀，帮助缓存命中"]
    align --> route["ContentRouter<br>按内容类型分流"]
    route --> a["SmartCrusher（JSON 等结构化数据）"]
    route --> b["CodeCompressor（代码）"]
    route --> c["base 压缩（一般文本）"]
    a --> cc["CCR 可逆压缩<br>压缩版进上下文"]
    b --> cc
    c --> cc
    cc -. "retrieve 按需取回细节" .-> llm["LLM"]
    llm --> cc
    cc --> agent
```

CacheAligner 负责稳定前缀、帮 Prompt Cache 命中；ContentRouter 判断内容类型，把 JSON、代码、文本分别交给 SmartCrusher、CodeCompressor、base 压缩器；CCR 负责可逆压缩，原始数据全部保留在本地。
...
更关键的是，压缩后的准确率没有下降。在 GSM8K（数学推理）、TruthfulQA（事实准确性）等标准基准上，压缩前后的得分基本持平甚至略优。
...
接入 CodeBuddy（用 [studyzy Fork 版](https://github.com/studyzy/headroom)，已增加 CodeBuddy 支持）：
...
两者方向不同，可以叠加使用。如果只装一个，RTK 解决最大的单点问题（终端输出）；如果追求更全面的压缩覆盖，再加上 headroom。
...
Agent 生成并运行脚本，只把结果（几行文字）放进上下文，而不是把 47 个文件的内容全部读进来。

**能力四：不干预输出格式**

context-mode 只管数据往哪走，不管 Agent 怎么回答。如果你同时装了 Caveman，两者各管各的，不冲突。

#### 安装

CodeBuddy 使用 [studyzy Fork 版](https://github.com/studyzy/context-mode)（已增加 CodeBuddy 支持）：
...
```
/context-mode:ctx-doctor    # 检查，所有项显示 [x] 即正常
...
Graphify 在 2026 年 4 月发布，10 天内 GitHub stars 超过 22k，如今已经超过 50.8k。

**核心机制**：用 Tree-sitter 解析代码构建知识图谱，AI 查图而不是反复读文件。

官方数据：**比直接读文件减少 71.5 倍 Token 消耗**。[12]
...
### 5.3 CodeGraph：持久化的图数据库方案

CodeGraph 是另一个方向的工具，基于 MCP Server + 持久化图数据库（Neo4j/KuzuDB），对 7 个真实开源仓库做了完整 benchmark（2026-08 复测，Claude Opus 4.8，每仓 4 轮取中值）：[13]

**平均数据**：**62% 更少 Token、88% 更少 Tool Call、44% 更低成本、53% 更快**。

**分仓库明细**（有图 vs 无图，回答同一个架构问题）：

| 代码库 | 语言 | Token | 工具调用 | 成本 |
| --- | --- | --- | --- | --- |
| VS Code | TypeScript | -77% | 2 vs 28 | -71% |
| Excalidraw | TypeScript | -84% | 2 vs 43 | -78% |
| Django | Python | -41% | 3 vs 14 | -13% |
| Tokio | Rust | -65% | 3 vs 29 | -64% |
| OkHttp | Java | -54% | 1 vs 6 | -21% |
| Gin | Go | -52% | 1 vs 7 | 持平 |
| Alamofire | Swift | -59% | 4 vs 33 | -57% |

**诚实的一面**：收益和项目规模、语言强相关。大型 Rust/TypeScript 项目收益最显著，中小 Java/Go 项目收益相对有限。而且官方明确承认：这些数字衡量的是"处理量"——过手的 token、花的钱、用的工具；**不衡量会话结束时上下文里残留多少**。长会话里，图返回的密集负载会留在上下文窗口，残留占用反而比"grep+读文件"式更高（7 个仓库平均约多 80%，VS Code 上是 67k vs 18k）。"处理更少、残留更多"同时成立，如果长期跑长 session，要为此留预算。
...
与 3.5 的连接：subagent 可以单独绑定便宜模型。规划用强模型，执行用便宜模型，两者在不同 session 里，互不干扰。
...
### 6.5 Orchestrator-Worker 模式：把多 Agent 协作变成工程
...
### 6.9 记忆外置：别让 session 承载全部历史

用"永不清空的超长会话"模拟记忆，是最贵也最脆弱的做法。

真正该长期保留的信息，外置出去：

**CODEBUDDY.md 体系**：项目约定、关键决策、架构边界。保持 < 50 行，是索引不是文档。详细内容放 `docs/`，让 Agent 按需读取。
...
到这里，这一层的心智模型也可以浓缩成一句话：**不要只想着把单次回答压短，更要想办法让系统少把同一批背景反复搬进来。**
...
再回头看这张路线图：从第一章到第六章，是一条"便宜 → 贵、易 → 难"的升级曲线。**从习惯开始，到路由，再进工具、图谱和多 Agent**，每一步都在为下一步铺路。先把今天能做的做了，成本数据一旦开始积累，优化就停不下来。
...
[4] Claude Code Prompt Cache 实战指南 —— https://cloud.tencent.com/developer/article/2662618
[5] Google AI for Developers，《Context caching - Gemini API》—— https://ai.google.dev/gemini-api/docs/caching
[6] Google AI for Developers，《Thinking》—— https://ai.google.dev/gemini-api/docs/thinking
[7] OpenAI，《使用 GPT-5.5 | OpenAI API》—— https://developers.openai.ac.cn/api/docs/guides/latest-model
[8] RTK（Rust Token Killer）GitHub —— https://github.com/rtk-ai/rtk
[9] RTK 实战指南 —— https://juejin.cn/post/7628503584259211316
[10] Caveman GitHub —— https://github.com/JuliusBrussee/caveman
[11] Caveman 论文：Brevity Constraints Reverse Performance Hierarchies —— https://arxiv.org/abs/2604.00025
[12] Graphify GitHub —— https://github.com/safishamsi/graphify
[13] CodeGraph GitHub —— https://github.com/colbymchenry/codegraph
[14] headroom GitHub —— https://github.com/chopratejas/headroom
[15] context-mode GitHub —— https://github.com/mksglu/context-mode
[16] LLM 推理成本工程：五层防御体系 —— https://jishuzhan.net/article/2062415217950273537
