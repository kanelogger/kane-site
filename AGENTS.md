...
- 页面、内容、配置或构建脚本变更后运行统一验证，它按顺序执行类型检查、构建、产物检查、场景预算和回归检查，失败即停止：

  ```sh
  pnpm verify
  ```

- `verify:build` 检查本地 `dist/` 的路由、链接、锚点、图片及 SEO / RSS 产物，必须先构建当前源码。
...
添加或迁移文章、作品及其本地图片时，以 [内容导入手册](docs/content-import.md) 为可移植入口；本地有 `kane-site-content-import` Skill 时配合使用。其过时 README 引用、字段或路径以公开手册和当前 Schema 为准。新 checkout 不要求安装私有 Skill。
